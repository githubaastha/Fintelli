    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        const currentThreshold = 
          ...
          percentageUsed >= 80 ? 80 : null;

        if (!currentThreshold) return;

        const lastThreshold = budget.lastAlertThreshold || 0;
        const isNewMonth = !budget.lastAlertSent || 
          isNewMonthCheck(new Date(budget.lastAlertSent), new Date());

        // Send if new month OR crossed a new higher threshold
        if (isNewMonth || currentThreshold > lastThreshold) {
          await sendEmail({
...
function isNewMonthCheck(lastAlertDate, currentDate) {
  return (
